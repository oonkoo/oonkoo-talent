...
export interface GlobeMarker {
  ...
  src: string;
  label?: string;
  /** Pixel width/height of the marker billboard image (default 8px). */
  size?: number;
}
...
export interface GlobeArc {
  ...
  /** Override the default arc color (config.arcColor). */
  color?: string;
}
...
export interface Globe3DConfig {
  ...
  /** Default arc color (used when an arc doesn't specify its own). */
  arcColor?: string;
  /** Default arc thickness in radius units. */
  arcThickness?: number;
  /** Arc opacity 0..1. */
  arcOpacity?: number;
}
...
interface Globe3DProps {
  ...
  markers?: GlobeMarker[];
  /** Array of arcs (start → end) drawn as raised great-circle curves */
  arcs?: GlobeArc[];
  /** Globe configuration */
    ...
    <group ref={groupRef} visible={isVisible}>
      ...
      <group ref={imageGroupRef} position={topPosition}>
        ...
        >
          <div
            className={cn(
              "cursor-pointer overflow-hidden rounded-full bg-none shadow-lg transition-transform duration-200",
...
// ============================================================================
// Arc Component (raised great-circle curve from start → end, with terminal dot)
// ============================================================================
...
function Arc({ start, end, radius, color, thickness, opacity }: ArcProps) {
  const { tubeGeometry, endDotPosition } = useMemo(() => {
    const startVec = latLngToVector3(start.lat, start.lng, radius * 1.001);
    const endVec = latLngToVector3(end.lat, end.lng, radius * 1.001);

    // Great-circle midpoint, raised above the surface proportional to arc length.
    const midDirection = startVec.clone().add(endVec).normalize();
    const arcAngle = startVec.angleTo(endVec); // radians
    const raise = radius * (0.18 + arcAngle * 0.18);
    const midpoint = midDirection.multiplyScalar(radius + raise);

    const curve = new THREE.QuadraticBezierCurve3(startVec, midpoint, endVec);
    return {
      tubeGeometry: new THREE.TubeGeometry(curve, 64, thickness, 8, false),
      endDotPosition: endVec,
    };
  }, [start, end, radius, thickness]);
...
function RotatingGlobe({
  ...
  return (
    <group ref={groupRef}>
      ...
      {/* Arcs — also inside the rotating group so they spin with the globe */}
      {arcs.map((arc, index) => (
        <Arc
          key={`arc-${index}-${arc.start.lat}-${arc.end.lat}`}
          start={arc.start}
          end={arc.end}
          radius={config.radius}
          color={arc.color ?? config.arcColor}
          thickness={config.arcThickness}
